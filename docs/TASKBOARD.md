...
**最後更新**：2026-09-18，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；TG-MVP-05 cumulative External Macro PASS / ACCEPTED（F1 = RESOLVED）；TG-MVP-06 進行中（B-98 Secret/Credential Hardening，candidate pending External Macro Audit）；NEXT_SLICE = TG-MVP-06A（待辦）；E-03 Roadmap 為詳細切片生命週期之唯一權威。
...
| TG-MVP-06 | 進行中 | **B-98 憑證與機密強化**（B-98 Secret/Credential Hardening）：在啟用真實 Telegram Bot Token 或 Local API HMAC secret 前，落實金鑰與機密存取安全邊界，防止 token 洩漏。 | 看板 B-98、安全防護原則 | TG-MVP-01 (G1) | TG-MVP-06 candidate 正在落地（secret-safe output binding rule, safe presence helper, staged scanner, pre-commit hook, CHECK 21）；PENDING EXTERNAL MACRO AUDIT |
