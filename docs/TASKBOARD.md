...
**NEXT_SLICE**：TG-MVP-07（B-30 + B-33 Gateway 固定通訊埠與顯式目標收斂）

**最後更新**：2026-09-18，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；TG-MVP-06 已完成（CLOSED）；B-101 進行中（IN PROGRESS）；TG-MVP-06A 進行中 / MACRO HOLD / PENDING EXTERNAL MACRO RE-AUDIT（TG-MVP-06A-F2 候選：F2-A production/live timeout parity、F2-B account-ID domain alignment）；NEXT_SLICE = TG-MVP-07（待辦，NOT STARTED / NOT AUTHORIZED）；E-03 Roadmap 為詳細切片生命週期之唯一權威。
...
| B-101 | 進行中 | **Gateway Secret Provider & Runtime Secret Consumption Boundary** | External Macro architecture decision 落地 Windows Credential Manager Generic Credential（CRED_TYPE_GENERIC, CRED_PERSIST_LOCAL_MACHINE）作為 Gateway v1 concrete secret provider。實作 provider-neutral contract（secret-provider.js）、Windows concrete provider（windows-credential-manager-provider.js）、Win32 CredReadW/CredFree PowerShell bridge（windows-credential-manager-read.ps1）、ADR-0026、單元測試與 Windows synthetic integration tests。滿足 exact-target lookup、零 store 列舉、零 env fallback、零 Local Config secret、零 AccountRegistry secretRef 突變、fixed named Windows-user identity invariant、runtime-only Buffer 曝光（best-effort consumer zeroization）、零 real credential access。首候選經 External Macro HOLD（Run 35336376440 success），TG-MVP-06A-F1 修復中（F1-A 原生指標 exactly-once 釋放防雙重 free、F1-B 受限同步 bridge 超時、F1-C 規範 SecretRef 凍結與目標重導權威邊界、F1-H1 PowerShell byte[] 最佳努力清除），修復候選待外部審計（PENDING EXTERNAL MACRO RE-AUDIT，不得標記已完成）。落地對應：E-03 Roadmap TG-MVP-06A。 |
