...
| TG-MVP-12 | 進行中 | **持久化 SQLite Outbox 實作**（Durable SQLite Outbox）：實作具備能力感知安全重試（Capability-Aware Safe Retry）之出站佇列，保證出站訊息不遺失，斷線時持久化保存。R0 candidate 4968e8d90c77e51e0745965a2376779a419d85b9 External Macro HOLD（Run 36212316461 attempt 1 verify PASS, gateway-windows PASS; findings F1–F3, accepted checkpoint fed1eeb888619138001969eb7b2f63ea3da33f6e）；R1 bounded repair in progress / awaiting re-audit；F1 LINE retry fail-closed、F2 missing transport phase MAY_HAVE_BEEN_SENT fail-closed、F3 GatewayRuntimeOwner OutboxWorker lifecycle 接線；repo-visible truth correction（ADR-0025 canonical 17 columns outbox schema, E24 Q01–Q48 raw 46f1079f...）；516 passed 0 failed 2 skipped；verify_all 全 Gates PASS；main advancement FORBIDDEN。 | 使用者裁決 R2、TG-MVP-03 (R2 ADR)、TG-MVP-04 (F1)、ADR-0025 | TG-MVP-03, TG-MVP-04, TG-MVP-11 | Schema v6 outbox 遷移完成、Capability-Aware Safe Retry Policy (R2) 實作完成、In-process Outbox Worker 實作完成、原子回覆授權與冪等重放完成、516 passed 0 failed 2 skipped、待外部宏觀審計裁決 |
| TG-MVP-13 | 待辦 | **Telegram 出站文字發送適配器**（Telegram outbound text delivery）：由 Outbox worker 取出待送文字訊息，透過 Telegram Bot API 送出並標記完成或重試。（註：D-R3 USER DECISION REQUIRED BEFORE TG-MVP-13 DESIGN；D-R2 deferred before TG-MVP-15）。 | 使用者裁決 D-U4（限文字 text in/out，排除附件與外發檔案）、ADR-0022、D-R3 | TG-MVP-10, TG-MVP-12 | 測試機器人文字回覆成功送達 Telegram 聊天室 |
