...
**最後更新**：2026-09-23，活動看板狀態請參見 NEXT_WORK 與 NEXT_SLICE。
...
| B-30 | 待辦 | **playwright 掃描清單含 3000／3001，運行風險已存在** | `skills/execution/playwright-automation/lib/helpers.js` 已由 TG-MVP-07 實作 explicit-target-only candidate contract，移除 built-in `commonPorts` 自動掃描，caller 必須明確指定 `targetPorts` 否則 fail closed；使用者回報之本機 port 5000 戰情儀表板（`USER_PROVIDED / NOT_NETWORK_PROBED / REPO_EXTERNAL`）記錄為既有 B-30 具體運行風險實例，不建新 task；待 External Macro Audit 審查結案（落地對應：E-03 Roadmap TG-MVP-07） |
...
| B-33 | 待辦 | **Port 規範的三個缺口** | current truth：root `.env.example` 已由 deterministic `git ls-files` discovery 確認 ABSENT，tracked env-like paths = NONE，不得重新建立 `.env.example`；保留真正剩餘 port-registry 收斂語意：ADR-0017 補齊 6379 Redis 與 9222／9223 Chrome CDP 已知 tooling ports，Gateway v1 canonical local port 3003 已由 TG-MVP-07 裁決收斂；待 External Macro Audit 審查結案（落地對應：E-03 Roadmap TG-MVP-07） |
...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 ACCEPTED / CLOSED；M2 ACCEPTED / CLOSED；M3 ACCEPTED / CLOSED；M4 LATER）。M3 final implementation SHA = 90b2b638fc4abf738568d8e1153f9834568e21ad，post-main Run = 35732469633（verify=success, gateway-windows=success，ALL 5 GATES PASSED）；B-109 overall NOT CLOSED because M4 remains later。採用 K4-A 單一寫入點收斂（docs/TASKBOARD.md 保持 remaining-work authority，docs/refactor-backlog.md §5.4 收斂為 pointer-only，state_authority_policy 與 runtime_surface_policy 落地）；B-106 Minimal Verified Runtime Kernel 落地結案；NEXT_WORK 為 E-03，NEXT_SLICE 為 TG-MVP-07；B-107 保持 OPEN；B-108 保持 OPEN；M4 保持 LATER；google-jules 不得於本輪 enable。 |
...
| TG-MVP-07 | 進行中 | **B-30 + B-33 Gateway 固定通訊埠與顯式目標收斂**（Gateway fixed port / explicit target convergence）：清理舊 repo 殘留之 port 3000/3001 衝突，確立 Gateway 專屬監聽 port 與目標端點規範。 | 看板 B-30、B-33、ADR-0022 | TG-MVP-07 candidate implementation 完成：Playwright 移除 implicit commonPorts 掃描改採 explicit targets only（fail closed）；ADR-0017 / ADR-0022 收斂 Gateway v1 canonical local port = 3003（no listener / no local config loader 實作）；SOP_04 / SOP_06 歷史敘述收斂；本機 port 5000 記錄為 USER_PROVIDED local occupant；Node 與 pytest 回歸測試通過；candidate awaiting External Macro review。 | Port 配置統一，無殘留衝突硬編碼 |
