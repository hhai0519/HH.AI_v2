...
| B-106 | 進行中 | **Actual Loaded-Surface Budget & Rule Compatibility Guard** | 進行中（B-106 candidate pending External Macro runtime evidence review）。B-106 Minimal Verified Runtime Kernel 與 Loaded-Surface Mechanical Guard 核心能力已於 B-109 M3 實作；root AGENTS.md 收斂為 minimal runtime kernel（<= 6000 字元，8 個 KERNEL-* 錨點與 1 個 canary），詳細技能架構下沉至 skills/AGENTS.md；test_rule_loadability 測試套件完成並包含 6 項確定性負例測試；IDE 2.5.5 環境基準對齊；runtime canary 驗證程序已建立，等待 External Macro 取得 USER_PROVIDED 執行期證據審查。B-106 保持 candidate 未關閉（NOT CLOSED）。 |
...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 ACCEPTED / CLOSED；M2 ACCEPTED / CLOSED；M3 repair candidate pending External Macro review；M4 LATER）。採用 K4-A 使用者正式裁決（Single-Write-Point State Convergence），docs/TASKBOARD.md 保持 remaining-work authority，docs/refactor-backlog.md §5.4 收斂為 pointer-only，state_authority_policy 與 runtime_surface_policy 於 rule-registry.json 落地；B-106 Minimal Verified Runtime Kernel 落地（root AGENTS.md 收斂為 <= 6000 字元核心安全閘門，8 個 KERNEL-* 錨點與 1 個 canary；skills architecture 下沉至 skills/AGENTS.md）；test_rule_loadability.py 提供負例測試；NEXT_WORK 為 B-109，NEXT_SLICE 為 Mechanical Governance v1 — M3；B-107 保持 OPEN；B-108 保持 OPEN；M4 保持 LATER；TG-MVP-07 保持 NOT AUTHORIZED。 |
