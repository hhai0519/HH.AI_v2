...
**最後更新**：2026-09-06，HEAD `e6f543a` 之後
...
| B-35 | 待辦 | **雙代理事實指紋與 Dashboard** | 階段一（排批 2）：`scripts/fingerprint.py` ＋ `scripts/anchor.py` ＋ 驗證腳本 `--quiet` ＋ CI 重跑同一腳本並逐位元比對。階段二三（待辦）：`docs/dashboard.html` ＋ GitHub Pages ＋ 接 CI。**階段二三刻意延後**——在假綠燈上建 Dashboard 會權威地顯示假綠燈 |
...
| B-37 | 待辦 | **`scripts/anchor.py` 錨點自動抽取器** | 輸入檔名與行範圍，輸出「原文 ＋ 行號 ＋ `count()` 值」。同時消滅「錨點未從 clone 逐字取得」（前任五次、現任一次）與「自檢聲明數量宣稱不符」（A-36、B-25）——機器抄的原文不漂移，機器數的數量不少算。排批 2 |
