...
| TG-MVP-08 | 進行中 | **T9 封存並退役舊版凍結 JSON 運作模組**（T9 retire frozen JSON operational modules）：退役舊版基於 JSON 檔案的通道狀態模組，全面轉移至 SQLite Repository。 | 看板 E-03、ADR-0022、ADR-0023 | TG-MVP-07A ACCEPTED / CLOSED | candidate 施工中（AWAITING EXTERNAL MACRO AUDIT / NOT CLOSED）：T9 frozen JSON operational modules retired from active repo；three frozen core modules deleted；three dedicated legacy suites deleted；stale DurableStateStore skip permission removed；Python structural anti-resurrection guard added；SQLite repository / tests VERIFY_ONLY；shared atomic primitive VERIFY_ONLY；no real user data migration。 |
