...
**最後更新**：2026-09-20，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；B-103A 已結案（ACCEPTED / CLOSED，Actions Run 35451619061 attempt 1 verify / gateway-windows success，ALL 5 GATES PASSED）；B-103B Phase B 通過（PASS / ACCEPTED，canary branch Run 35452195334 same-SHA capability proof PASS）；B-103B Phase C 通過（PASS / ACCEPTED，ruleset 21301111 pull_request rule removed，deletion / non_fast_forward / strict required_status_checks verify + gateway-windows preserved，bypass empty）；B-103B Phase D1 已結案（ACCEPTED / CLOSED，PR #30 squash-merged，post-main Run 35456270854 attempt 1 verify / gateway-windows success，ALL 5 GATES PASSED，new transport contract ACTIVE ON MAIN）；B-103B Phase D2 進行中（SAME-SHA PROOF CANDIDATE PENDING EXECUTION / MACRO AUDIT，建立 batch/b103d2-same-sha-proof，batch checks 通過後以 GitHub update_ref force=false 推進 main 至同一 SHA）；正式新增治理任務 B-106、B-107、B-108 待辦（TODO BLOCKING BEFORE PHASE E）；TG-MVP-01B 進行中（B-103 ACTIVE）；TG-MVP-07 尚未授權（NOT AUTHORIZED）。
...
| B-100 | 待辦 | **跨領域營運與治理殘留收斂（Cross-Cutting Operational / Governance Residual Closure）** | 橫向殘留傘狀任務（NONBLOCKING，不阻擋 TG-MVP-02 / B-103A）。涵蓋五大子項：R-A NotebookLM Smart App Control；R-B Actions 證據耐用性與假綠燈保全（Actions Evidence Durability & False-Green Preservation，NONBLOCKING，涵蓋關鍵事故 raw log 最小決定性 evidence preservation、一次性掃描執行當時由 GitHub API machine-enumerated 的所有 conclusion=success Verify runs 之 canonical failure markers、以及機器確立 Actions retention setting；B-103A 不執行 full scan、不修改 retention，B-100 R-B 不阻擋 B-103A）；R-C 執行者可見語言合約；R-D CI 供應鏈可重現性；R-E 歷史權威檢視。 |
...
| B-103 | 進行中 | **C-06 Exact-SHA 快速推進閘門遷移（Exact-SHA Fast-Forward Gate Migration）** | IN PROGRESS（B-103A ACCEPTED / CLOSED，Actions Run 35451619061 attempt 1 success；B-103B Phase B PASS / ACCEPTED，canary Run 35452195334 same-SHA capability proof PASS；B-103B Phase C PASS / ACCEPTED，ruleset 21301111 pull_request rule removed，deletion / non_fast_forward / strict required_status_checks preserved，bypass empty；B-103B Phase D1 ACCEPTED / CLOSED，PR #30 squash-merged 產生 cf2a2b9，post-main Run 35456270854 attempt 1 verify / gateway-windows success，new transport contract ACTIVE ON MAIN；B-103B Phase D2 SAME-SHA PROOF CANDIDATE PENDING EXECUTION / MACRO AUDIT；B-106/B-107/B-108 REGISTERED TODO BLOCKING BEFORE PHASE E；不得宣稱 Phase D2 完成、不得宣稱 B-103 closed、不得宣稱 Phase E authorized）。 |
...
| B-105 | 待辦 | **Jules Mode A 風險掃描啟用（Jules Mode A / Risk-Scan Enablement）** | 待辦（D-U12, D-U16）；Jules 僅為 proposer、不得直接寫 repo；Mode A risk scan 為 primary，Mode B patch proposal deferred；不安裝 Antigravity Jules Bridge / Send2Jules（D-U13）；重啟前置條件：B-10 協作規範、@google/jules-mcp 版本鎖定（ADR-0019 decision 4）、CHECK 21 維持有效。 |
| B-106 | 待辦 | **Always-On Rule Budget & Guarded Rule Mutation** | 待辦（TODO / BLOCKING BEFORE PHASE E）。目前 scripts/tests/test_rule_loadability.py 對全部 .agents/rules/*.md 只有 MAX_RULE_CHARS = 12000 hard ceiling，僅 prompt-preflight.md 另有 SAFETY_TARGET_CHARS = 9500。歷史觀察：git-and-reporting.md 曾於 mutation 出現 12750/12000 UI warning 後始精簡；核心問題為現行機制屬 post-write rejection 而非 pre-write architectural budget guard。未來 repair 目標：character-based budget、all Always-On rules coverage、operational safety headroom、remaining chars / headroom percentage reporting、prospective / temporary-content validation，PASS 才允許正式 replacement，避免超額寫入後壓縮成為常態。權威單位為 Unicode characters，禁以 UTF-8 bytes 替代。NOT IMPLEMENTED。 |
| B-107 | 待辦 | **Deterministic Consistency Diagnostics & Guarded State-Log Mutation** | 待辦（TODO / BLOCKING BEFORE PHASE E）。check_consistency.py 目前 final summary 僅顯示「通過 N 項, 失敗 M 項」，未列 FAILED CHECK IDs / names，容易因畫面末尾 CHECK22 產生誤導。未來 repair 至少要求：deterministic result registry、summary 明確顯示 FAILED checks (N): CHECK <id> — <name>，以及 structured Markdown state 尤其 docs/EXEC-LOG.md 使用 guarded append / prospective validation，避免自由文字 mutation 先產生 invalid table state。歷史 transient failure 與 CHECK20 Markdown table continuity 修復一致，但因未保留 replayable evidence，記載僅限 INFERRED / CONSISTENT WITH CHECK20，禁記 CONFIRMED CHECK20。NOT IMPLEMENTED。 |
| B-108 | 待辦 | **Unauthorized Destructive Local Git Reset Guard** | 待辦（TODO / BLOCKING BEFORE PHASE E）。prior D1 Executor 於 PR #30 merge 後執行 git checkout main && git reset --hard origin/main，違反 active rules 明文禁止未授權 reset --hard 及破壞性 Git 操作 S1 規範。remote repository 未產生 mutation，但 local tracked-data-loss impact 確立為 NOT_ESTABLISHED（不得宣稱 DATA LOSS CONFIRMED 亦不得宣稱 NO IMPACT）。未來 hardening 至少確保：destructive_git_allowed=false 不僅為 prompt prose，而能由架構／機械守衛阻擋把 reset --hard 作為常態 sync 操作。常態 sync 必須使用 git fetch、ref comparison 與 non-destructive 分支/worktree 操作。NOT IMPLEMENTED。 |
...
| C-07 | 已裁決 | **歷史待辦 B-17 / B-28 / B-29 處置決策（ARCHIVE vs TRIGGER_BASED_DEFERRED）** | **2026-09-18 使用者裁決**：採 `TRIGGER_BASED_DEFERRED`（依觸發條件延後，不封存）。適用於 B-17（章節語意變更偵測）、B-28（`AGENTS.md` 上游規範版本記錄）、B-29（上游一致性對照表與機械檢查）。不作 active production work，僅於其既定上游／規則架構觸發條件實際發生時重啟評估。不自行發明新觸發條件。 |
