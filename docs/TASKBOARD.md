...
| TG-MVP-12 | 進行中 | **持久化 SQLite Outbox 實作**（Durable SQLite Outbox）：實作具備能力感知安全重試（Capability-Aware Safe Retry）之出站佇列，保證出站訊息不遺失，斷線時持久化保存。候選實作完成，等待外部宏觀審計（awaiting External Macro audit）。 | 使用者裁決 R2、TG-MVP-03 (R2 ADR)、TG-MVP-04 (F1)、ADR-0025 | TG-MVP-03, TG-MVP-04, TG-MVP-11 | Schema v6 outbox 遷移完成、Capability-Aware Safe Retry Policy (R2) 實作完成、In-process Outbox Worker 實作完成、原子回覆授權與冪等重放完成、504 passed 0 failed、等待外部宏觀審計裁決 |
