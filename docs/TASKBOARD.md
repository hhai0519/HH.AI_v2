...
**NEXT_SLICE**：TG-MVP-06A（Gateway Secret Provider & Runtime Secret Consumption Boundary）

**最後更新**：2026-09-18，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；TG-MVP-05 cumulative External Macro PASS / ACCEPTED（F1 = RESOLVED）；TG-MVP-06 進行中（B-98 Secret/Credential Hardening，079f candidate Actions Run 35327963222 machine/CI success，F1-A/F1-B RESOLVED，External Macro HOLD，TG-MVP-06-F2 exit-code evidence drift correction in progress）；NEXT_SLICE = TG-MVP-06A（待辦，NOT AUTHORIZED）；E-03 Roadmap 為詳細切片生命週期之唯一權威。
...
| B-101 | 待辦 | **Gateway Secret Provider & Runtime Secret Consumption Boundary** | TG-MVP-06 discovery 已確認：Gateway SecretProvider = ABSENT，concrete provider = UNDECIDED；MCP Windows User env guidance 不治理 Gateway。ADR-0022 D26：禁止 Token / Channel Secret plaintext local config。ADR-0025：Agent 不應直接看到 Local API HMAC secret。因此在任何 real-secret consumer 前必須獨立決定：concrete secret provider、account → secret reference model、exact-name retrieval、runtime-only value exposure、failure semantics、Windows security boundary、test strategy、no real secret in CI。明確：TG-MVP-06 不裁決 provider，由本任務獨立閉合（落地對應：E-03 Roadmap TG-MVP-06A）。 |
...
> - **D-U2 / D-U6 / D-U7**：`USER DECIDED / G2 LANDING PENDING`（個人資料不進 repo、分級審查、MISSION 切換定義）。
...
| TG-MVP-06 | 進行中 | **B-98 憑證與機密強化**（B-98 Secret/Credential Hardening）：在啟用真實 Telegram Bot Token 或 Local API HMAC secret 前，落實金鑰與機密存取安全邊界，防止 token 洩漏。 | 看板 B-98、安全防護原則 | TG-MVP-01 (G1) | TG-MVP-06 079f candidate machine/CI success，F1-A/F1-B RESOLVED，External Macro HOLD（TG-MVP-06-F2）；repair candidate 落地中（F2 exit-code evidence drift correction）；PENDING EXTERNAL MACRO RE-AUDIT |
| TG-MVP-06A | 待辦 | **Gateway 機密提供者與執行期機密取用邊界**（Gateway Secret Provider & Runtime Secret Consumption Boundary）：基於 B-101、ADR-0022 D26、ADR-0025 §15，選定並落實 concrete repo-external / OS-backed Gateway secret provider 與 runtime consumption boundary。本批不實作。 | B-101、ADR-0022 D26、ADR-0025 §15 | TG-MVP-06 | provider architecture explicitly decided；no plaintext Local Config；no secret in AccountRegistry；no Agent-visible value；exact-name / handle-based retrieval；synthetic-only automated tests；real secret not required by CI；ready for Telegram / HMAC consumer integration |
