...
**NEXT_SLICE**：Legacy Jules PR / Branch Cleanup
...
**最後更新**：2026-09-21，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；B-109 M1 External Macro Final PASS 結案狀態同步（B-109 M1 ACCEPTED / CLOSED，M1-F1 RESOLVED，M1 pre-push guard 首次 production 使用實證通過）；B-104 狀態更新為 READY / AUTHORIZED DIRECTION（本 closure batch 不得執行 B-104）；NEXT_WORK 推進至 B-104，NEXT_SLICE 為 Legacy Jules PR / Branch Cleanup；B-104 完成後返回 B-109 M2；TG-MVP-07 仍不得開始。
...
| B-104 | 待辦 | **舊版 Jules PR 與分支清理（Legacy Jules PR / Branch Cleanup）** | 待辦（READY / AUTHORIZED DIRECTION；B-109 M1 ACCEPTED，解除延後鎖定；本 closure batch 不得執行 B-104）；清理標的嚴格限定於使用者明確授權之 open PR #13–#24 及其 12 條 exact current head branches（U-A）；執行前 External Macro 重新查 PR #13–#24 狀態與 exact SHA，任何 drift 則全批停止；需搭配 REMOTE_DELETE_EXACT_SET 機器授權；因 IDE Advanced Command Access 將 `--delete` 設為 Deny，執行 B-104 前需使用者本機步驟暫時將 `--delete` 改為 Allow，執行唯一授權 delete command 後改回 Deny 並截圖供審計官核對；標準 transport 採 `git push origin --delete`，禁止 refspec 繞道；其他 legacy remote branches 未授權，不納入 B-104；完成 B-104 後返回 B-109 M2。 |
...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 ACCEPTED / CLOSED；M1-F1 RESOLVED；M2 PENDING；M3 PENDING；M4 LATER）。承接使用者正式裁決 U1、U3、K2-A、K4-C。M1 已由 External Macro Auditor 判定 ACCEPT ALL，accepted checkpoint 推進至 20f1afb60a0ad9a07575664db6e5fc5bf786e10e；M1 pre-push 機械守衛已於 production main promotion 實證驗證且單次授權成功消耗。NEXT_WORK 推進至 B-104，NEXT_SLICE 為 Legacy Jules PR / Branch Cleanup，完成 B-104 後返回 B-109 M2。Phase roadmap：M1（CLOSED）、M2（plan-vs-actual、CI diff replay、B-107 evidence-integrity hardening）、M3（actual loaded-surface governance、B-106、state convergence、K4 cutover decision）、M4（skill exploration、Jules、capability semantics）。包含 MAX_AUTO_PLAN_REVISIONS = 3、escalation A–F、DEGRADED state、governance versioning 與 drift monitor。 |
