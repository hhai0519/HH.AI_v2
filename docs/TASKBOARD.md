...
| TG-MVP-11 | 進行中 | **本機迴路 Local API v1 實作**（Loopback Local API v1）：實作僅限 127.0.0.1 監聽、具備 HMAC 簽章驗證之 HTTP 入口，供本機 Agent 發送出站回覆與查詢狀態。 | 使用者裁決 R3、TG-MVP-03 (R3 ADR)、TG-MVP-06 (B-98)、TG-MVP-04 (F1)、TG-MVP-07A、使用者綁定裁決 D-TG11-3 | TG-MVP-03, TG-MVP-04, TG-MVP-06, TG-MVP-06A, TG-MVP-07A | 進行中（IN PROGRESS / R1 BOUNDED REPAIR, AWAITING EXTERNAL MACRO）；實作 127.0.0.1 綁定、HMAC 驗證、回覆授權與客戶端 CLI 包裝器；前置客戶端缺陷已由有界根因排除修復，T1–T23 突變測試 23/23 valid semantic RED；候選 0901e06b... 經 External Macro 審查判定 HOLD，成立 F1–F8；使用者正式裁決 D-TG11-3（初始活躍 Telegram 帳號規則採恰好一個啟用帳號，0 或 >=2 則 fail closed 並輸出 GATEWAY_CONFIG_ERROR，禁止陣列順序或隨機選取）；R1 修正生產引導、金鑰提供者、請求主體限制、嚴格協定文法、HMAC 後記帳、回應協定驗證、Content-Type 嚴格驗證、有界 stderr 診斷；R1-M1～R1-M9 反事實突變測試 9/9 valid semantic RED，單元測試、迴歸測試全數 PASS，等待外部宏觀審計。 |
