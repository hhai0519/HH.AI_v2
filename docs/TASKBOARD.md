...
| TG-MVP-10 | 進行中 | **Telegram 測試機器人入站適配器**（Telegram Test-Bot inbound adapter）：實作 Telegram 長輪詢（long polling）入站適配器，將 Telegram update 轉換為標準 inbox 訊息並呼叫 `ingestMessage()`。 | ADR-0022、TG-MVP-02 (Identity ADR)、TG-MVP-06 (B-98) | TG-MVP-02, TG-MVP-05, TG-MVP-06, TG-MVP-06A | 進行中（BOUNDED REPAIR R2 / AWAITING R2 CANDIDATE / EXTERNAL MACRO AUDIT）；R0 ddd8fcbb Actions Run 36025143966 success，Macro HOLD，Claude non-authoritative advisory HOLD；R2 授權有界修復；task plan revision_count = 2，Allowed 25 檔；落實金鑰 Buffer 直接採納與停用/終態歸零、活躍帳號熱切換守衛、嚴格重試白名單、固定 Telegram 端點/常數、嚴格結構語法、非文字更新耐久 IGNORED 處置；反事實突變證明 10/10 RED；待建立 R2 候選提交與外部宏觀審計。不得標記 ACCEPTED/CLOSED，不得推進 TG-MVP-11。 |
