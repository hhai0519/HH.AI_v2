...
| B-21 | 待辦 | **CHECK 16 編號被兩件事佔用** | B-14 已將 CHECK 16 實作為 EXEC-LOG 落後偵測，B-12 仍登記 CHECK 16 為 `audited-*` tag 落後偵測。B-12 改編號 CHECK 17；**惟 CHECK 17 已被 B-40 的 ADR 規範混雜偵測預定，B-12 順延為 CHECK 19**。根因是規劃編號與實作編號無單一權威來源 |
...
| B-24 | 待辦 | **§6.7 第七次失效：改字串未搜尋其引用位置** | 審計官修改 FAIL 訊息時未搜尋該字串在測試中的斷言，並錯誤判定「不改變行為」。§6.7 的搜尋對象清單需增列「被測試斷言引用的字串」，依 §5.7 同步三處 |
| B-25 | 已完成 | **自檢聲明 E11 數量宣稱不符** | 聲明「22 個錨點」實列 21 個。A-36 同一形狀第二次。**機制有效**——執行者依 §3.4 逐條清點後推翻。比照 A-38 不新增規則，僅留紀錄 |
| B-26 | 待辦 | **攔截點缺少執行時點** | 驗證步驟要求 `grep "^| " docs/EXEC-LOG.md` 應為 3 行，但同批依 §3.8 追加紀錄列後變 4 行。時點不明的攔截點兩個答案都能自圓其說。`auditor-protocol.md` §6.5 只規範形式未規範時點 |
| B-27 | 待辦 | **「動手前必讀」機制從未被驗證，三份規則檔有兩份虛構** | 2026-09-06 實測：執行者貼出的 `prompt-preflight.md` 與 `git-and-reporting.md` 章節標題、§1 全部條目、§2 整節內容皆與實際檔案不符（`prompt-preflight.md` 那張 14 列「歷史失效清單」表在實際檔案中不存在）；§3.4 表漏 E12 卻聲稱驗證了 E12。`role-boundaries.md` 屬實。**回報與實際不符的第六類：規則來源虛構**，最嚴重，因為後續所有檢查都建立在被虛構的規則上。處置：本批提示詞已加入章節序列比對；根本解法為 B-35 指紋機制 |
...
| B-35 | 進行中 | **雙代理事實指紋與 Dashboard** | 階段一（排批 2）：`scripts/fingerprint.py` ＋ `scripts/anchor.py` ＋ 驗證腳本 `--quiet` ＋ CI 重跑同一腳本並逐位元比對。階段二三（待辦）：`docs/dashboard.html` ＋ GitHub Pages ＋ 接 CI。**階段二三刻意延後**——在假綠燈上建 Dashboard 會權威地顯示假綠燈 |
...
| B-39 | 待辦 | **獨立性的來源從審計官轉移到 CI** | 擴充 ADR-0007 立論：原則要求的是「獨立性」，不是「必須由 Claude 提供獨立性」。CI 在 GitHub runner 重跑同一腳本並比對，是雙方都控制不了的第三方。**落點為 `PRINCIPLES.md` §0.5 ＋ `auditor-protocol.md` §1／§6.1 ＋ `handover-selftest.md` D 節，不開 ADR**（原提議開 ADR-0020，即 §5.7 第七次，見 B-40）。三條紅線：判斷不下放、獨立 clone 不取消、CI 綠燈仍須每批抽樣反例驗證。排批 2 |
| B-40 | 待辦 | **`PRINCIPLES.md` §1 的判別演算法缺審計官出口** | 層級表未列 `.claude/rules/`（907 行），四個判別問句的答案也沒有它——第 2 問只給執行者的兩個目的地。任何「給審計官的規則」依序自問只能落到第 4 問（ADR）。**現任審計官即因此提議開 ADR-0020，是 §5.7 第七次。這是演算法缺陷不是個別疏忽。** 本批已修 §1；**CHECK 17（ADR 規範混雜偵測）排批 2**——實測 19 份 ADR 有 10 份 Decision 區塊含祈使句且無分層搬移標記，其中 ADR-0017（4 處）正是 C-01 衝突的成因、ADR-0013（10 處）正是 C-03 要拆的那份 |
...
| B-42 | 待辦 | **CHECK 16 靜默跳過無法解析的列，且 docstring 與實作不符** | 正規表示式 `[0-9a-fA-F]+\|BOOTSTRAP` 無法匹配「本批」，該列被完全忽略且**不輸出任何 INFO**。docstring 稱「讀最後一列」，實作是「讀最後一個能解析的列」——B-11 的第三次。依 §9.3 每批末列恆為「本批」，故此解析失敗是常態非例外。2026-09-06 實測：`rows` 為 `["BOOTSTRAP","e6f543a"]`，PASS 是巧合不是設計。處置：無法解析的列必須輸出 INFO 說明跳過幾列與原因；docstring 改為與實作一致。排批 2b |
| B-43 | 待辦 | **CHECK 15 的 pending 集合在正常流程恆為空** | 實測回報「已核對 12 個 hash、待核對 0 個」。§9.3 要求當批項目符號寫「本批（尚未 commit）」不帶 hash，故 pending 永遠是 0。它仍能抓 A-18 第 5 次的半途狀態，但「待核對 0 個」與「一切正常」在輸出上分不出來。處置：輸出改為明確區分「無待核對項目（正常）」與「有待核對項目且無衝突」。排批 2b |
...
| B-50 | 待辦 | **`prompt-preflight.md` §3.4 的交叉驗證可被審計官以「知情偏離」宣告繞過** | 2026-09-06 審計官在 2a-fix 把 E8 標為 ⚠️ 並附理由，執行者接受並未停止，導致 CHECK 12 在 CI 上 FAIL。§3.7 明寫「十八項全部可機械驗證，沒有任何一項需要你憑信任接受」，但 E8 就此變成信任項——與該節建立時要消滅的 E11 造假是同一個洞。而 `role-boundaries.md` §2 又禁止執行者判斷規範是否應存在，執行者被夾在中間。**修法：E8 比對為否時一律停止、不接受任何理由；審計官需偏離 §6.1-8 時，唯一合法路徑是先另開一批修改規則本身。** 排批 2b |
| B-51 | 待辦 | **判準含 HEAD 的檢查，本地與 CI 的答案結構性差 1** | 本地在 commit 前跑、CI 在 commit 後跑。CHECK 8／9 門檻 `lag > 2` 故差 1 看不出；**CHECK 12／16 門檻 `lag > 1`，正好卡在差值上**——「漏更新 AUDIT-LOG」在本地驗證中原理上偵測不到。2026-09-06 實證：本地 HEAD=`2d76958` lag=1 PASS、CI HEAD=`3a85a30` lag=2 FAIL。**執行者無疏忽。** 處置方向：這四項增加 `--as-if-committed` 本地預演模式，或另設 pre-commit 的等效判準。排批 2b |
| B-52 | 待辦 | **同形錯誤連續三批：改動或跳過某物之前，未查誰依賴它** | 批 1 改 `check_consistency.py` 顯示字串未查測試斷言引用；批 2a 設計跨平台位元組比對未查 git 跨平台轉換；批 2a-fix 宣告偏離 §6.1-8 未查其機械守衛 CHECK 12。`auditor-protocol.md` §6.7 已有此規則但措辭只舉「數字」「章節引用」為例。**處置：§6.7 的搜尋對象清單擴為四類**——被測試斷言引用的字串／會被執行環境改動的東西（換行、編碼、路徑分隔符）／有機械守衛的規則／本地與 CI 判準會分歧的東西。依 §5.7 同步進 §6.1、selftest E 節、preflight §3 三處。**原 B-24 併入本項** |
...
| B-57 | 待辦 | **審查機制的完成定義（五條可機械驗收）** | ①每個 CHECK 是獨立函式且有正反例測試（現 **9/16**）②每個 CHECK 至少命中過一次真違規並留紀錄（現**未追蹤**，處置為指紋 schema v2 增 `checks.last_hit`）③每份規範層檔案至少被一個 CHECK 涵蓋（現 **SOP/ 未涵蓋**）④本地與 CI 判準一致（現**差 1**，見 B-51）⑤審計官不能豁免任何自檢項（本批已修）。**現況 0/5，本批後 1/5** |
...
| B-60 | 待辦 | **CHECK 17–24 編號一次配置完成（防 B-21 重演）** | B-21 記錄過「CHECK 16 編號被兩件事佔用」，根因是規劃編號與實作編號無單一權威來源。2026-09-06 一次配完：**17** ADR 規範混雜（B-40，批 2d）／**18** 規則三處同步（B-50，批 2d）／**19** `audited-*` tag 落後（B-12、B-21，批 2d）／**20** ARCHIVE-INDEX 可達性（B-59，批 2d）／**21** 跨層矛盾（B-54，批 2e）／**22** CRLF ＋ BOM（B-46、B-31，批 2d）／**23** 文件時效（B-55、B-56，批 2e）／**24** 跨技能觸發詞重疊（B-32，批 4）。**本列即為編號的單一權威來源，新增 CHECK 前必須先讀本列。** B-21 併入本項 |
| B-61 | 待辦 | **證據區塊 (d) 的圍欄數沒有被納入 §3.6 的交叉驗證** | `.agents/rules/prompt-preflight.md` §3.6 的驗證表只有三項：區塊存在／行數相符／結構相符，**圍欄數不在其中**。2026-09-06 實測：審計官在提示詞把 `.claude/rules/auditor-protocol.md` 的圍欄數誤寫為 0（實際為 2，位於第 54、65 行），執行者回報實際值但判定為「零變動相符」——它比對的是修改前後，不是與宣稱值。**這不是執行者疏漏，是 §3.6 沒有要求它比對。** 一個沒有人核對的數字等於沒有寫。處置：§3.6 驗證表增列圍欄數比對。排批 2d |
...
| B-72 | 待辦 | **`PRINCIPLES.md` §2.10「每一個數字都必須有它的產生者」** | 使用者 2026-09-06 提出「每次宣告的數據都親自從 GitHub 重新取得」，審計官逐項檢驗 16 件錯誤後修正範圍：**只能修 1 件、部分修 3 件、修不掉 12 件**——審計官本來就每輪重 clone，多數錯誤不是讀取問題而是「寫了沒有產生者的數字」。正確規則為：**寫進提示詞的每一個數字，都必須是本輪某次工具呼叫的輸出**。四種合法來源：repo 現況→新鮮 clone；文件的自我描述（清單長度、項數）→腳本 `len()`；套用後的未來狀態→本地模擬；外部系統狀態→該系統 API。**「從 GitHub 重讀」只是第一種。** 排批 2b-4 |
| B-73 | 待辦 | **`§8.4-2` 的「每輪最多一次 clone」是上限，沒有下限**（**使用者發現**） | 審計官每輪重 clone 靠自律不靠規則，下一個接手者可能用開場那次 clone 撐三輪。處置：`.claude/rules/auditor-protocol.md` §6.1 新增一項補下限「HEAD 可能移動即必須重 clone」，並要求自檢聲明能指出每個數字由哪一次工具呼叫產生。依 §5.7 同批落到 `handover-selftest.md` 與 `prompt-preflight.md` §3.4。排批 2b-4 |
| B-74 | 待辦 | **執行者只回報無法從 commit 重生的東西**（**使用者發現**） | CHECK 輸出、行數、圍欄數、測試結果、指紋皆可由審計官自行重生，不必貼進對話。只回報四類：commit hash、CI 結論、停止原因、暫態實測（反例注入後已還原、repo 中查不到的）。**停止時只貼「不符項目 ＋ 相關檔的尾三條 INFO」**——實證：某次停止貼 136 行，審計官是靠其中 9 行下的結論。**例外**：動手前必讀的章節序列比對必須照貼，它測的是執行者有沒有真的讀檔（B-27），不是 repo 狀態。排批 2b-4 |
...
| B-76 | 待辦 | **審計官的核對階段未腳本化** | 每輪固定跑 clone → `validate_skills` → `check_consistency` → `pytest` → `fingerprint --verify` → `git diff --stat` → 結構確認，而審計官每次手打。批 2a 即因 `--verify` 不在「驗證三項」內而漏跑（B-47）。處置：`scripts/audit_verify.py <base_head>` 一條指令跑完並輸出報告骨架，**內建 B-47 把 `--verify` 列為第四項**。排批 2b-4 |
| B-77 | 待辦 | **BPE 的輸出不得被審計官修改** | B-64 的直接推論：BPE 產生證據後若「順手修潤措辭」，就重現「模擬的對象不等於送出的文字」。處置：`.claude/rules/auditor-protocol.md` §6.1 第 18 項延伸一句——**BPE 輸出後不得修改，要改就改批次規格再重跑**。排批 2b-4 |
| B-78 | 待辦 | **`scripts/apply_batch.py`：消除散文這一層有損的重新編碼** | 現行流程中，審計官寫散文提示詞、執行者解析散文並自己寫一次性套用腳本——**兩份腳本、兩次解析、中間隔著一次散文編碼**。若改為同一份批次規格「審計官用它模擬、執行者用它套用」，則「模擬的對象＝送出的文字＝執行者套用的文字」三者**在結構上不可能不一致**，B-64 那類錯誤從「靠閘門攔下」變成「不可能發生」。與 BPE 共用同一個解析器。排批 2b-5 |
