...
**最後更新**：2026-09-21，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；B-109 M1 External Macro HOLD / BOUNDED EVIDENCE REPAIR ACTIVE（8776 candidate 技術實作與 CI 經 External Macro 驗證無新 material code finding，因 M1-F1 EXEC-LOG factual evidence drift 進行 bounded evidence repair）；NEXT_WORK 仍為 B-109，NEXT_SLICE 仍為 Mechanical Governance v1 — M1；不得開始 B-104 或 M2。
...
| B-104 | 待辦 | **舊版 Jules PR 與分支清理（Legacy Jules PR / Branch Cleanup）** | 待辦（AUTHORIZED DIRECTION / EXECUTION DEFERRED UNTIL B-109 M1 PASS）；清理標的嚴格限定於使用者明確授權之 open PR #13–#24 及其 12 條 exact current head branches（U-A）；執行前 External Macro 重新查 PR #13–#24 狀態與 exact SHA，任何 drift 則全批停止；需搭配 REMOTE_DELETE_EXACT_SET 機器授權；因 IDE Advanced Command Access 將 `--delete` 設為 Deny，執行 B-104 前需使用者本機步驟暫時將 `--delete` 改為 Allow，執行唯一授權 delete command 後改回 Deny 並截圖供審計官核對；標準 transport 採 `git push origin --delete`，禁止 refspec 繞道；其他 legacy remote branches 未授權，不納入 B-104；本輪絕不執行 B-104。 |
...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 MACRO HOLD / BOUNDED EVIDENCE REPAIR ACTIVE）。承接使用者正式裁決 U1、U3、K2-A、K4-C。8776 candidate 之 technical implementation 及 exact-SHA CI 已由 External Macro 驗證無新 material code finding；blocking item 為 M1-F1 EXEC-LOG evidence-integrity correction（F1-A manifest finding_disposition、F1-B original M1 E24 required/executed/replayed、F1-C fingerprint path exec-latest.json）。NEXT_WORK 仍為 B-109，NEXT_SLICE 仍為 Mechanical Governance v1 — M1，不得開始 B-104 或 M2。Phase roadmap：M1（minimal Rule Registry、shared Execution Contract parser、PASS/FAIL + Rule ID Preflight、.githooks/pre-push exactness guard、actual synthetic Git hook invocation REG、K6 persistent truth 更正、environment recovery checklist、TASKBOARD recovery trigger、git switch -c 分支慣例、CHECK 25、credential/env/session/browser/goal-pressure REG、B-104 路由延後）、M2（plan-vs-actual、CI diff replay、B-107 evidence-integrity hardening）、M3（actual loaded-surface governance、B-106、state convergence、K4 cutover decision）、M4（skill exploration、Jules、capability semantics）。包含 MAX_AUTO_PLAN_REVISIONS = 3、escalation A–F、DEGRADED state、governance versioning 與 drift monitor。 |
