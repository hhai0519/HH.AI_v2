...
**最後更新**：2026-09-19，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；B-102 / INCIDENT-CI-05 已結案（CLOSED / RESOLVED）；B-103 進行中（PHASE A — BATCH CHECK BOOTSTRAP / B-103A REPAIR ACTIVE，現行維持 protected PR 傳輸，future target 才是 fast-forward SAME SHA to main）；TG-MVP-01B 進行中（B-102 RESOLVED / B-103 ACTIVE）；TG-MVP-07 尚未授權（NOT AUTHORIZED）；登錄 D-U9 至 D-U16、U-A、U-B、B-104、B-105 與 B-100 R-B；候選待 External Macro Pre-Merge Audit。
...
| B-103 | 進行中 | **C-06 Exact-SHA 快速推進閘門遷移（Exact-SHA Fast-Forward Gate Migration）** | IN PROGRESS（Phase A — Batch Check Bootstrap / B-103A REPLACEMENT REPAIR ACTIVE）；擴充 verify.yml 納入 batch/** push 觸發事件且保留 PR 觸發；B-103A 不啟用新 main transport，本輪 merge 前後生產傳輸維持 protected PR mode；future target 才是 batch branch → exact-SHA checks → fast-forward SAME SHA to main；Phase C ruleset 變更尚未授權。登錄 Actions attempt 引用需求（run_id + attempt）與 rerun/cancel authority matrix 需求。 |
