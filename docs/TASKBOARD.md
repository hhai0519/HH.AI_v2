...
**最後更新**：2026-09-14，B-68 Dependency Closure Phase 1 Micro-Fix In Progress / Next: B-68
...
| B-68 | 進行中 | **`scripts/impact_scan.py`** | PRE-B01 / governance reliability。建立 deterministic tracked-repo dependency discovery + evidence replay，將 dependencies-before-Allowed-Scope 從 Auditor 記憶規則提升為 executable path。Phase 1 initial candidate 2eca machine PASS，但 External Macro counterexample 確認 F1-F4。bounded micro-fix 修正：BPE fail-closed、complete query/result closure、UPDATE ⊆ Allowed Scope machine pairing、canonical CLI truth。micro-fix candidate 仍需 External Macro audit 與 runtime malformed-scope canary。B-41/B-01 待辦 (NOT STARTED)。 |
