...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 ACCEPTED / CLOSED；M2 ACCEPTED / CLOSED；M3 implementation candidate pending External Macro review；M4 LATER）。採用 K4-A 使用者正式裁決（Single-Write-Point State Convergence），docs/TASKBOARD.md 保持 remaining-work authority，docs/refactor-backlog.md §5.4 收斂為 pointer-only，state_authority_policy 與 runtime_surface_policy 於 rule-registry.json 落地；B-106 Minimal Verified Runtime Kernel 落地（root AGENTS.md 收斂為 <= 6000 字元核心安全閘門，8 個 KERNEL-* 錨點與 1 個 canary；skills architecture 下沉至 skills/AGENTS.md）；test_rule_loadability.py 提供負例測試；NEXT_WORK 為 B-109，NEXT_SLICE 為 Mechanical Governance v1 — M3；B-107 保持 OPEN；B-108 保持 OPEN；M4 保持 LATER；TG-MVP-07 保持 NOT AUTHORIZED。 |
