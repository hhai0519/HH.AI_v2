...
| B-100 | 待辦 | **跨領域營運與治理殘留收斂（Cross-Cutting Operational / Governance Residual Closure）** | 橫向殘留傘狀任務（NONBLOCKING，不阻擋 TG-MVP-02 / B-103A）。涵蓋六大子項：R-A NotebookLM Smart App Control；R-B Actions 證據耐用性與假綠燈保全（Actions Evidence Durability & False-Green Preservation，NONBLOCKING，涵蓋關鍵事故 raw log 最小決定性 evidence preservation、現行 134 筆 success runs 一次性掃描 canonical failure markers、以及機器確立 Actions retention setting；B-103A 不執行 full scan、不修改 retention，B-100 R-B 不阻擋 B-103A）；R-C 執行者可見語言合約；R-D CI 供應鏈可重現性；R-E 歷史權威檢視。 |
