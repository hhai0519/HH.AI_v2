...
**NEXT_SLICE**：TG-MVP-01B（進行中，B-102 RESOLVED / B-103 TRANSPORT MIGRATION ACTIVE）

**最後更新**：2026-09-20，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；B-103A 已結案（ACCEPTED / CLOSED，Actions Run 35451619061 attempt 1 verify / gateway-windows success，ALL 5 GATES PASSED）；B-103B Phase B 通過（PASS / ACCEPTED，canary branch Run 35452195334 same-SHA capability proof PASS）；B-103B Phase C 通過（PASS / ACCEPTED，ruleset 21301111 pull_request rule removed，deletion / non_fast_forward / strict required_status_checks verify + gateway-windows preserved，bypass empty）；B-103B Phase D1 已結案（ACCEPTED / CLOSED，PR #30 squash-merged，post-main Run 35456270854 attempt 1 verify / gateway-windows success，ALL 5 GATES PASSED，new transport contract ACTIVE ON MAIN）；preserved D2 機器證據完整保留；使用者正式授權 K1-A、K2-A、K3-A、K4-C、K5-A、K6-A 與 U1/U2/U3 裁決；T1-bootstrap successor 外部審計狀態為 PRE-PROMOTION MACRO HOLD / BOUNDED REPAIR ACTIVE（修復 T1-F1～T1-F4 四項發現，落實 K1-A transport-neutral contract，更正 active-state projection，native pinned-SHA adapter 尚未 production-proven，不得宣稱 T1 結案）；B-28 / B-29 因使用者 U2 裁決正式重啟（REOPENED BY USER U2 / PENDING / NOT IMPLEMENTED）；B-106 / B-107 / B-108 範圍校準，新增 B-109 Mechanical Governance v1 待辦（NOT IMPLEMENTED）；TG-MVP-01B 進行中（B-103 ACTIVE / K1 TRANSPORT TRANSITION）；TG-MVP-07 尚未授權（NOT AUTHORIZED）。
...
| B-100 | 待辦 | **跨領域營運與治理殘留收斂（Cross-Cutting Operational / Governance Residual Closure）** | 橫向殘留傘狀任務（NONBLOCKING，不阻擋 TG-MVP-02 / B-103A）。涵蓋五大子項：R-A NotebookLM Smart App Control；R-B Actions 證據耐用性與假綠燈保全（Actions Evidence Durability & False-Green Preservation，NONBLOCKING；關鍵事故 raw log 最小決定性 evidence preservation；current retention setting = MUST MACHINE-VERIFY before relying on deadline，critical evidence 必須在實際 retention expiry 前 preservation，未取得 machine evidence 前禁止將 exact retention duration 寫成 current fact）；R-C 執行者可見語言合約；R-D CI 供應鏈可重現性；R-E 歷史權威檢視。 |
...
| B-103 | 進行中 | **C-06 Exact-SHA 快速推進閘門遷移（Exact-SHA Fast-Forward Gate Migration）** | IN PROGRESS（B-103A ACCEPTED / CLOSED；B-103B Phase B PASS / ACCEPTED；B-103B Phase C PASS / ACCEPTED；B-103B Phase D1 ACCEPTED / CLOSED，PR #30 squash-merged 產生 cf2a2b9，post-main Run 35456270854 verify / gateway-windows success，new transport contract ACTIVE ON MAIN；preserved D2 evidence commit 131a0b19cdd915d6172a750972c69fdb1afdadd9 機器證據保留；K1-A / K5-A 使用者授權；T1 bootstrap candidate 1ee1d41 External Macro HOLD，當前狀態為 PRE-PROMOTION MACRO HOLD / BOUNDED REPAIR ACTIVE，修復 T1-F1～T1-F4 四項審計發現；native pinned full-SHA adapter 尚未 production-proven；B-106/B-107/B-108 範圍校準，新增 B-109 待辦；不得宣稱 T1 PASS、不得宣稱 B-103 closed、不得宣稱 Phase E authorized、不得宣稱 TG-MVP-01B closed）。 |
| B-104 | 待辦 | **舊版 Jules PR 與分支清理（Legacy Jules PR / Branch Cleanup）** | 待辦（NOT AUTHORIZED in B-103A）；清理標的嚴格限定於使用者明確授權之 open PR #13–#24 及其 machine-derived 之 12 條 exact current head branches（U-A）；嚴禁因看起來舊而自行納入其他 branch；本輪不關閉 PR、不刪除分支；留待獨立 bounded destructive task 執行。 |
...
| B-107 | 待辦 | **Deterministic Consistency Diagnostics & Guarded State-Log Mutation** | 待辦（TODO / BLOCKING BEFORE PHASE E）。擴充為 evidence integrity owner：EVIDENCE-ORIGIN 確立 MACHINE_CAPTURED_RAW、MACHINE_DERIVED、AGENT_ASSERTED、USER_PROVIDED 四類，verification_status 必須獨立於 evidence_origin；納入 PATH-EXISTENCE、GENERATOR-IN-BUNDLE、REPORT-TRACEABILITY 與 REG-11～REG-13 routing；記錄 E24 v2 instance finding：raw discovery = VALID（SHA-256 edfe3a4），但 hit-summary manifest 指向未包含於 ZIP 之 generate_hit_summary.py，pre_post_invariant.json 混雜 raw capture 與 derived comparison 卻標為 MACHINE_CAPTURED_RAW，External Macro 獨立重算 summary 一致，raw dependency evidence 可用但 bundle traceability not clean；此為 EXISTING B-107 承接，不另立重複項目；check_consistency summary 列出 FAILED checks 與 structured log guarded append 規範保持。NOT IMPLEMENTED。 |
| B-108 | 待辦 | **Unauthorized Destructive Local Git Reset Guard** | 待辦（TODO / BLOCKING BEFORE PHASE E）。擴充登錄 K6-A 權限態勢：保留 Full access / Always Proceed，但 destructive / main-push 指令納入 Deny list、GitHub MCP write tools 移除 permanent Allow、移除舊 repo `.env.local` write allow、移除不存在之 `.gemini\antigravity\skills` read allow、browser GitHub 操作不得 unconditional allow；cross-chat history 僅為 context 而非 production authority，production-impact fact 必須由 current repo/Git/GitHub 重新驗證；禁止 production batch 中途更新 IDE；Hooks 於 IDE 2.5.5 下為 DOCUMENTED_YES / RUNTIME_NO / CAUSE_UNKNOWN，不作 M1 enforcement layer；本輪不修改 IDE settings；destructive_git_allowed=false 機械防護保持。NOT IMPLEMENTED。 |
| B-109 | 待辦 | **Mechanical Governance v1** | 待辦（NOT IMPLEMENTED）。承接使用者正式裁決 U1、U3、K2-A、K4-C。Phase roadmap：M1（minimal Rule Registry、task-specific Execution Contract、PASS/FAIL + Rule ID Preflight、incident-derived REG、K6、transport guard、active-state projection guard）、M2（plan-vs-actual、CI diff replay、B-107 evidence-integrity hardening）、M3（actual loaded-surface governance、B-106、state convergence、K4 cutover decision）、M4（skill exploration、Jules、capability semantics）。包含 MAX_AUTO_PLAN_REVISIONS = 3、escalation A–F、DEGRADED state、governance versioning 與 drift monitor；本 T1 僅實作兩個 minimal guards，不提前實作完整 M1–M4。 |
...
> **架構決策狀態**：
> - **R2（安全重試 ＋ 持久化 Outbox）**：`USER DECIDED / CANONICAL ARCHITECTURE LANDED IN ADR-0025 / NOT IMPLEMENTED`。
> - **R3（Loopback HTTP v1 127.0.0.1 HMAC 授權）**：`USER DECIDED / CANONICAL ARCHITECTURE LANDED IN ADR-0025 / NOT IMPLEMENTED`（Named Pipe 未獲選/延後）。
...
| TG-MVP-01B | 進行中 | **G2 權威規則落地與 C-06 預防性 GitHub Gate**（G2 Authority Landing & Preventive GitHub Gate）：落實使用者已裁決之治理與架構規則；B-102 事故已修復結案（B-102 RESOLVED / INCIDENT-CI-05 CLOSED / R6 RESOLVED）；B-103 傳輸遷移進行中（B-103 TRANSPORT MIGRATION ACTIVE；D1 transport contract 已 active on main，現正由 T1 bootstrap 轉為 transport-neutral K1 invariant，T1 native pinned-SHA adapter 尚未 production-proven）；待 B-103 exact-SHA 快速推進傳輸遷移完成與 External Macro 驗收；不得標記 CLOSED；TG-MVP-07 仍為 NOT AUTHORIZED。 | 使用者裁決 D-U2、D-U6、D-U7、C-06 Option B、C-07、C-08、B-100 R-D | TG-MVP-06A (ACCEPTED / CLOSED) | G2 治理文件、CHECK 22 與遠端 GitHub Ruleset 落地驗證 |
...
| TG-MVP-07 | 待辦 | **B-30 + B-33 Gateway 固定通訊埠與顯式目標收斂**（Gateway fixed port / explicit target convergence）：清理舊 repo 殘留之 port 3000/3001 衝突，確立 Gateway 專屬監聽 port 與目標端點規範。 | 看板 B-30、B-33、ADR-0022 | TG-MVP-01B External Macro closure（NOT AUTHORIZED until TG-MVP-01B closed） | Port 配置統一，無殘留衝突硬編碼 |
...
| TG-MVP-10 | 待辦 | **Telegram 測試機器人入站適配器**（Telegram Test-Bot inbound adapter）：實作 Telegram 長輪詢（long polling）入站適配器，將 Telegram update 轉換為標準 inbox 訊息並呼叫 `ingestMessage()`。 | ADR-0022、TG-MVP-02 (Identity ADR)、TG-MVP-06 (B-98) | TG-MVP-02, TG-MVP-05, TG-MVP-06, TG-MVP-06A | Test Bot 入站訊息成功持久化至 SQLite inbox，游標正確推進 |
| TG-MVP-11 | 待辦 | **本機迴路 Local API v1 實作**（Loopback Local API v1）：實作僅限 127.0.0.1 監聽、具備 HMAC 簽章驗證之 HTTP 入口，供本機 Agent 發送出站回覆與查詢狀態。 | 使用者裁決 R3、TG-MVP-03 (R3 ADR)、TG-MVP-06 (B-98)、TG-MVP-04 (F1)、TG-MVP-07A | TG-MVP-03, TG-MVP-04, TG-MVP-06, TG-MVP-06A, TG-MVP-07A | Local API HTTP 伺服器單元與整合測試，驗證 127.0.0.1 綁定、HMAC 驗證與回覆授權 |
...
| TG-MVP-13 | 待辦 | **Telegram 出站文字發送適配器**（Telegram outbound text delivery）：由 Outbox worker 取出待送文字訊息，透過 Telegram Bot API 送出並標記完成或重試。 | 使用者裁決 D-U4（限文字 text in/out，排除附件與外發檔案）、ADR-0022 | TG-MVP-10, TG-MVP-12 | 測試機器人文字回覆成功送達 Telegram 聊天室 |
