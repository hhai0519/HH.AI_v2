...
**NEXT_SLICE**：Mechanical Governance v1 — M4 Governance Preflight Fail-Fast CLI Hardening
...
**最後更新**：2026-09-25，活動看板狀態請參見 NEXT_WORK 與 NEXT_SLICE。
...
| B-109 | 進行中 | **Mechanical Governance v1** | 進行中（M1 ACCEPTED / CLOSED；M2 ACCEPTED / CLOSED；M3 ACCEPTED / CLOSED；M4 IN PROGRESS / IMPLEMENTATION CANDIDATE / AWAITING EXTERNAL MACRO AUDIT）。M3 final implementation SHA = 90b2b638fc4abf738568d8e1153f9834568e21ad，post-main Run = 35732469633（verify=success, gateway-windows=success，ALL 5 GATES PASSED）；M4 實作 governance_preflight.py 顯式互斥操作模式（--prompt-file, --verify-push, --create-main-auth, --create-delete-auth），徹底移除裸呼叫之隱式 stdin fallback，解決 Antigravity 背景執行 open stdin 懸掛根因；涵蓋 open-pipe regression 測試、反事實突變金絲雀證明、規則文件更新；待 External Macro 審查；main advancement = FORBIDDEN。採用 K4-A 單一寫入點收斂（docs/TASKBOARD.md 保持 remaining-work authority，docs/refactor-backlog.md §5.4 收斂為 pointer-only，state_authority_policy 與 runtime_surface_policy 落地）；B-106 Minimal Verified Runtime Kernel 落地結案；current routing 以本檔頂部 NEXT_WORK / NEXT_SLICE 欄位為唯一動態權威；B-107 保持 OPEN；B-108 保持 OPEN；google-jules 不得啟用。 |
