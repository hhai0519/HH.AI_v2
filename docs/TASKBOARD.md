...
**NEXT_SLICE**：Mechanical Governance v1 — M1

**最後更新**：2026-09-21，ACTIVE_MACRO_AUDITOR = GPT 代理審查官（使用者授權）；T1 same-SHA 生產晉升完成（ACCEPTED / CLOSED，Run 35517555366 verify 與 gateway-windows 通過，ALL 5 GATES PASSED）；B-103 正式結案（ACCEPTED / CLOSED）；TG-MVP-01B 正式結案（ACCEPTED / CLOSED）；使用者更新 K5-A 決策；K6-A 權限態勢已配置套用（Always Proceed，9 項 Deny 清單，基準文件建立）；登錄安全與流程事故圍堵事實，路由至 B-108、B-109 與 B-107；NEXT_WORK 推進至 B-109 M1；TG-MVP-07 待 B-109 M1-M3 完成後始得重啟。
...
| B-107 | 待辦 | **Deterministic Consistency Diagnostics & Guarded State-Log Mutation** | 待辦（TODO / BLOCKING BEFORE PHASE E）。擴充為 evidence integrity owner：EVIDENCE-ORIGIN 確立 MACHINE_CAPTURED_RAW、MACHINE_DERIVED、AGENT_ASSERTED、USER_PROVIDED 四類，verification_status 必須獨立於 evidence_origin；納入 PATH-EXISTENCE、GENERATOR-IN-BUNDLE、REPORT-TRACEABILITY 與 REG-11～REG-13 routing；新增 nonblocking 觀察：1. F2 negative control 目前僅證明 extracted inventory != expected，未獨立證明完整 inventory checker 對 missing/duplicate ID 本身 deterministic FAIL；2. CHECK 9 verdict parser 依 summary substring 判定 PASS/HOLD 屬結構弱點；3. 本次 promotion incident 形成 secret-bearing execution trace / report-surface provenance 案例；全部路由 B-107 與 B-109 M1/M2；本輪不修改 verifier。NOT IMPLEMENTED。 |
| B-108 | 待辦 | **Unauthorized Destructive Local Git Reset Guard** | 待辦（TODO / NOT IMPLEMENTED，作為 mechanical guard / cleanup owner）。K6-A UI posture 已更新為 APPLIED（evidence_origin = USER_PROVIDED，IDE 2.5.5，recorded 2026-09-20，完整基準見 docs/ops/antigravity-environment-baseline.md）。保留 Full access / Always Proceed；Deny list 目標包含：git branch -D、git checkout、git clean、git commit --amend、git credential、git rebase、git reset、git restore、git stash；git push 不在 Deny list；已移除 Desktop-wide read allow、chrome-devtools-mcp/evaluate_script permanent Allow、GitHub MCP write tools（GitHub server 端 disabled 18，保留 24/42 enabled）、Execute URLs 移除 github.com、新 GCM 認證已由使用者完成；保留歷史事故實例（git checkout <path>、git credential fill、GCM OAuth credential exposure、unauthorized secret env access、unauthorized PAT read/use、cross-session transcript access、promotion proceeded before K6 completion was externally verified）；登錄低優先 Legacy Antigravity permission cleanup 留待 future bounded cleanup。 |
| B-109 | 待辦 | **Mechanical Governance v1** | 待辦（NEXT WORK / M1 READY）。承接使用者正式裁決 U1、U3、K2-A、K4-C。Phase roadmap：M1（minimal Rule Registry、task-specific Execution Contract、PASS/FAIL + Rule ID Preflight、incident-derived REG、K6、transport guard、active-state projection guard；M1 擴充：1. mechanical promotion authorization verification；2. credential-access / secret-boundary REG；3. Executor 不得為取得 remote evidence 突破 secret boundary；4. raw GitHub Actions job log 由 External Macro 驗證；5. Executor 無安全 evidence channel 回報 UNKNOWN / DEFER_TO_EXTERNAL_MACRO；6. cross-session transcript access detection；7. K6 posture verification；8. goal-pressure boundary bypass REG family 防護）、M2（plan-vs-actual、CI diff replay、B-107 evidence-integrity hardening）、M3（actual loaded-surface governance、B-106、state convergence、K4 cutover decision）、M4（skill exploration、Jules、capability semantics）。包含 MAX_AUTO_PLAN_REVISIONS = 3、escalation A–F、DEGRADED state、governance versioning 與 drift monitor。 |
...
| TG-MVP-07 | 待辦 | **B-30 + B-33 Gateway 固定通訊埠與顯式目標收斂**（Gateway fixed port / explicit target convergence）：清理舊 repo 殘留之 port 3000/3001 衝突，確立 Gateway 專屬監聽 port 與目標端點規範。 | 看板 B-30、B-33、ADR-0022 | K2-A 治理執行排序：依序完成 B-109 M1 → M2 → M3 後，始得返回 product runtime mainline 重啟 TG-MVP-07 | Port 配置統一，無殘留衝突硬編碼 |
