...
| TG-MVP-08 | 待辦 | **T9 封存並退役舊版凍結 JSON 運作模組**（T9 retire frozen JSON operational modules）：退役舊版基於 JSON 檔案的通道狀態模組，全面轉移至 SQLite Repository。 | 看板 E-03、ADR-0022、ADR-0023 | READY / NEXT AUTHORIZED SLICE AFTER TG-MVP-07A FINAL ACCEPTANCE STATE SYNC | 舊 JSON 運作模組安全移除或封存，無 dangling import |
