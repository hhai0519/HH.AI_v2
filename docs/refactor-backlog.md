...
- 本批（尚未 commit）（CHECK 15 名稱收斂 ＋ §9.3 hash 規則 ＋ §5.6 反例測試規則
  ＋ §6.6 錨點選擇規則 ＋ §6.1 第 13／14／15 項 ＋ selftest E15／E16／E17
  ＋ preflight §3.1 新配對規則與 §3.5 ＋ 補打兩個 audited tag
  ＋ 十四項缺口登錄）已執行，**尚待審計官核對**，見 §5.4。
  本批曾八度被執行者攔下，全部發生在寫入或提交之前，repo 未受汙染。
  其中三次特別值得記錄：**插入位置逆序違反的是本批剛寫進 §6.6 的規則**；
  **§3.2 覆蓋規則首次實際攔到東西，攔到的是規則作者本人**；
  **執行者做了規則未要求的沙盒預演**，發現跨檔引用因換行斷開會觸發
  CHECK 10——那正好違反本批寫進 selftest E16 的規則。
  詳見第 42 點 G 至 M 段。
...
- **本批（CHECK 15 名稱收斂 ＋ 三條新規則 ＋ 補打 tag ＋ 六項缺口登錄）
  已執行完成，等待審計官核對。**
  核對通過後，下一批要做：更新 §5.1 的 HEAD、打 `audited-<本批 hash>`、
  以及執行 B-12（CHECK 16：`audited-*` tag 落後偵測）。
- **D-01 E1 注入測試進行中**：題 1 已於 2026-09-05 通過。
  依 C-05 裁決，判定者為現任審計官、題目由使用者保管。
  題 2 需另開乾淨對話（題 1 的對話已知道正確數字，會被污染）。
- **交接前必做的四項（B-02／B-07／B-08／B-09）已於 `0213568` 完成，
  本批修補其中的實作缺陷。** 剩下的交接前置只有 D-01。
