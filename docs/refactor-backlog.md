...
上次核對通過的 HEAD：eb40749

- `eb40749`（任務看板 `docs/TASKBOARD.md` ＋ §6.5 攔截點 ＋ §10 任務板維護）
  已於 2026-09-02 由審計官核對通過：2 改 1 新檔、零夾帶、驗證三項全過，
  兩個攔截點（章節序列、看板項目數）皆未觸發。
- 同日執行全盤盤點（兩個 repo），發現看板漏了整個重構主線與四個追蹤項、
  審計官第一輪的十項 HANDOVER 發現從未落地、backlog 編號斷號造成兩處死引用、
  舊 repo 三個區塊從未盤點。詳見第 32 點。
- 本批（看板補完至 41 項 ＋ 死引用修正 ＋ `ARCHIVE-INDEX.md`）已執行，
  **尚待審計官核對**，見 §5.4。

### 5.2 待辦（依優先序）

> **完整任務看板（含已完成項目與封存建議）見 `docs/TASKBOARD.md`。**
> 本節只列當前最高優先的待辦，兩者若不一致以 `TASKBOARD.md` 為準。
...
- **本批（看板補完）已執行完成，等待審計官核對。**
  核對通過後，請把 §5.1 第一行的 HEAD 更新為本批的 commit hash。
- **下一批是 `TASKBOARD.md` F-07**：`docs/HANDOVER.md` 十項過期與不一致
  一次修完。該批完成後才進入 E1 注入測試。
- E2 的任務是 `TASKBOARD.md` B-01（ADR-0002／0004／0010 分層搬移）——
