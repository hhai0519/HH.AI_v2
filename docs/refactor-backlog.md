...
23. **ADR-0007 分層搬移、`.claude/` 目錄建立、第三次回報虛構（2026-09-01）**
    ...
    **這推翻了 2026-08-29 的一項裁決**（當時依「指令句密度僅 1 句」
    ...
    （見第 21 點），不足以作為分類指標；改用 `PRINCIPLES.md` §1 的
...
上次核對通過的 HEAD：c238fd2

- `c238fd2`（§9.1 驗證階段例外、§9.4 判定者依情境區分、§9.5 驗證階段）
  已於 2026-09-02 由審計官核對通過：2 檔變更零夾帶、驗證三項全過、圍欄偶數。
- 同日審計官對交接機制做**兩次獨立複驗**（第一次逐節讀原文、
  第二次以結構錨點反查），找出四個缺口：交接前未檢查本機與遠端同步、
  品質影響評估未進 repo、缺可重複的接手自檢清單、
  §6.1 必備要素漏了「更新交接區」。四項已於本批修正。
- 本批（四缺口補正 ＋ 建立 `handover-selftest.md`）已執行，
  **尚待審計官核對**，見 §5.4。

### 5.2 待辦（依優先序）

1. **ADR-0002／0004／0010 分層搬移**（目標層皆已存在，不需裁決）
   - ADR-0002 第 19-24 行 bucket 觸發積極度分級 → `AGENTS.md` §5
     （該 ADR 的 Consequences 第 28 行自己就要求寫進 `AGENTS.md` §5，至今未落地）
   - ADR-0004 第 30-36 行「資料來源失效不得悄悄替代」普適原則
     → `.agents/rules/skills-architecture.md`（現 24 行）
   - ADR-0010 第 43-55 行 PowerShell 參數傳遞三條
     → `.agents/rules/powershell-encoding-protocol.md`（現 57 行）
     **注意**：該規則檔目前只談編碼、不談參數傳遞，兩者不重疊，屬追加不是覆蓋。
   - 實際檔名為 `0002-skill-invocation-aggressiveness.md`、
     `0004-notebooklm-fallback-rule-recovery.md`、
     `0010-powershell-parameter-passing-pitfalls.md`
2. **`check_consistency.py` 增補三項檢查**：檔尾換行、簡體字、setext 標題誤判。
   這是唯一「投入一批、之後每批都省」的項目，優先度僅次於第 1 項。
3. 新建 `SOP/SOP_03_Skill_Lifecycle_and_Quality.md`：
   收納舊 `SOP_00_Skill_Lifecycle` §一／§三／§四與舊 `SOP_03` §4.2／§4.3
   （詳見第 18 點）
4. `validate_skills.py` 增加「description 是否含觸發詞」警告（不是錯誤，
   因現存多個技能會失敗）＋ 對應測試
5. DLP 裝飾樣板存量清理（25 份 SKILL.md，見第 17 點）
6. `description` 引號寫法收斂（54 個技能中 26 加引號、28 未加，見第 20 點 C）
7. 產出 `.claude/rules/rule-traceability.md` 規則追溯表：
   逐條列出每一條規則的「寫在哪個檔案／誰會讀到／什麼機制讓它一定被觸發」。
   第三欄填不出來的規則即為死規則，須修正或刪除。
   （注意：接手自測清單刻意**不放進 repo**——那是驗證新 Agent 行為的考題，
    寫進 repo 等於讓受測者先看到答案。該清單由使用者保管。）
...
- **本批（交接機制四缺口補正）已執行完成，等待審計官核對。**
  核對通過後，請把 §5.1 第一行的 HEAD 更新為本批的 commit hash。
- **核對通過後隨即進入 §9.5 的驗證階段**：先 E1 注入測試（拋棄式對話），
  通過後才 E2 正式交接。
- E2 的任務是 §5.2 第 1 項（ADR-0002／0004／0010 分層搬移）——
  **該項在 E2 之前不得由現任 Agent 先行完成**，否則 E2 沒有真實任務可測。
- 驗證期間適用 §9.1 的不交接例外，現任審計官全程留任並擔任判定者。
- 規則追溯表（§5.2 第 7 項）刻意排在 E1／E2 之後：
  追溯表要涵蓋的規則清單會因測試結果而變動，先測再做表比較有效率。
