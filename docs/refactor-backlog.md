...
- 本批（尚未 commit）（`scripts/fingerprint.py` ＋ `scripts/anchor.py`
  ＋ 兩者測試 ＋ CI 指紋比對 ＋ B-42 至 B-45 登錄）已執行，
  **尚待審計官核對**，見 §5.4。
...
- **本批（`scripts/fingerprint.py` ＋ `scripts/anchor.py` ＋ 兩者測試
  ＋ CI 指紋比對 ＋ B-42 至 B-45 登錄）已執行完成，等待審計官核對。**
  核對通過後，下一批（批 2b）要做：CHECK 17（ADR 規範混雜偵測）、
  CHECK 18（規則三處同步）、驗證腳本 `--quiet`、B-39 落點三處同步、
