...
- 本批（尚未 commit）（B-72 `PRINCIPLES.md` §2.10 ＋ B-73 §6.1 第 19 項
  與 E21 ＋ B-77 §6.1-18 延伸句）已執行，**尚待審計官核對**，見 §5.4。
...
- **本批（B-72 §2.10 ＋ B-73 §6.1 第 19 項與 E21 ＋ B-77 §6.1-18 延伸句）
  已執行完成，等待審計官核對。** 本批為 2b-4，純規則層、零新程式，
  **證據區塊由 `scripts/build_prompt_evidence.py` 產生**。
- **給下一位審計官**：本批的 AUDIT-LOG 列包含 `936b9af` 的完整核對紀錄
  （含兩項反例注入的重跑結果），該核對原本只存在於對話中。
  **先執行本批，否則那份核對會遺失。**
- **批 2b-5**：B-74（執行者只回報無法從 commit 重生的東西，
  落點 `.agents/rules/git-and-reporting.md` 新增 §2.4）
  ＋ **B-80／B-81（BPE 的兩個限制）** ＋ `scripts/impact_scan.py`。
- **B 節 52 個待辦需要先做取捨**，見第 52 點 E 段。
  本批為 2b-3，一支新程式、零規則變更。
- **2b-3 是最後一份手寫的提示詞。** 2b-4 起，提示詞的證據區塊
  由 BPE 產生；依 B-77，**BPE 的輸出不得被審計官修改，
  要改就改批次規格再重跑**。
- **批 2b-4**：`scripts/impact_scan.py` ＋ 其測試 ＋ BPE 加呼叫點
  ＋ B-72／B-73／B-74／B-77 四條規範 ＋ B-75 樣板抽出
  ＋ B-76 `audit_verify.py`。**其提示詞由 BPE 產生，是它的第一次實戰。**
- **批 2b-5**：B-78 `apply_batch.py`。
- **批 2b-3**（下一批）：B-47（`fingerprint.py --verify` 升格為標準
  驗證集第四項）、B-49（`scripts/ci_status.py`，**必須含 badge SVG
  fallback**——GitHub API 每小時 60 次的上限曾兩度擋住審計官）、
  `scripts/pre_removal_check.py`（`PRINCIPLES.md` §2.9 三步的機械化）、
  **B-37（讓 `scripts/anchor.py` 產生提示詞的錨點清單，
  消滅審計官六度數錯錨點數量的問題）**。
- **後續**：批 2c（B-53 CHECK 1–7 抽成函式並補正反例測試）→
  批 2d（CHECK 17／18／19／20／22 ＋ B-42／B-43／B-48／B-51）→
  批 2e（CHECK 21 跨層矛盾 ＋ CHECK 23 文件時效）→
  批 2f（B-58 減法：HANDOVER 拆解、A 節封存、backlog 分冊）→
  批 3（B-01）→ 批 4（C-01～C-03 ＋ B-30～B-33）→
  批 5（上游對照表）→ 批 6（Runtime 依賴調研）→ 批 7 起重構主線。
- **批 2b-2**：B-47（`--verify` 升格標準驗證集）、
  B-49（`scripts/ci_status.py` 含 badge fallback）、
  `scripts/pre_removal_check.py`（`PRINCIPLES.md` §2.9 三步的機械化）、
  **B-62（審計官發出含規則層變更的提示詞前，必須在本地套用並跑
  `check_consistency.py`）**。
- **後續**：批 2c（B-53 CHECK 1–7 抽成函式並補正反例測試）→
  批 2d（CHECK 17／18／19／20／22 ＋ B-42／B-43／B-48／B-51／B-61）→
  批 2e（CHECK 21 跨層矛盾 ＋ CHECK 23 文件時效）→
  批 2f（B-58 減法：HANDOVER 拆解、A 節封存、backlog 分冊）→
  批 3（B-01）→ 批 4（C-01～C-03 ＋ B-30～B-33）→
  批 5（上游對照表）→ 批 6（Runtime 依賴調研）→ 批 7 起重構主線。
- **批 2b 的優先順序已重排**：原規劃以 CHECK 17／18 為主，
  改為先修 B-50 與 B-52——那兩項是讓審計官不能再犯同樣錯誤的機制，
  其餘都是在那之上疊加的功能。
- **批 2 已拆為 2a 與 2b**。理由：原規劃 12 檔加兩支全新腳本，
  而新腳本沒有舊版可 diff，核對成本高於改既有檔，
  依 `.claude/rules/auditor-protocol.md` §8.4-2 超出可驗證上限。
- **本節曾落後三批**（`875a604`／`ec840fe`／`e6f543a` 期間皆未更新），
  2026-09-06 由新審計官核對時發現。CHECK 9 只驗證 §5.1 第一行的 HEAD，
  本節沒有任何落後偵測。登錄為 B-22。
- **執行順序已定案（2026-09-06 使用者裁決）**：批 1 假綠燈與 §1 修正 →
  批 2 指紋與機械化 → 批 3 B-01 三份 ADR 分層搬移 → 批 4 C-01／C-02／C-03
  執行 ＋ B-30／B-31／B-32／B-33 → 批 5 上游對照表 → 批 6 Runtime 依賴調研
