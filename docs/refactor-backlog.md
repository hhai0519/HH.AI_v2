...
上次核對通過的 HEAD：85a8358

- `85a8358`（§9.1 交接提示詞完整模板、§9.4 交接驗收四項判準）
  已於 2026-09-02 由審計官獨立 clone 核對通過：變更範圍 2 個檔案、零夾帶，
  驗證三項全過，`---` 佔位符零殘留，開場指令端到端實測成功
  （交接區實際回傳 58 行有效內容）。
  核對時發現兩處文件與現行安排矛盾（§9.4 誤植判定者、§9.1 缺驗證階段例外），
  已於本批修正。
- 本批（§9.1 例外條款、§9.4 判定者修正、§9.5 驗證階段）已執行，
  **尚待審計官核對**，見 §5.4。
...
- **本批（交接機制驗證階段的規則補正）已執行完成，等待審計官核對。**
  核對通過後，請把 §5.1 第一行的 HEAD 更新為本批的 commit hash。
- **核對通過後隨即進入 §9.5 的驗證階段**，依序執行 E1 注入測試、
  E2 正式交接。E2 的任務是 §5.2 第 1 項（ADR-0002／0004／0010 分層搬移）——
