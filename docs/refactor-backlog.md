...
- 本批（尚未 commit）（補上前一批被跳過的三處更新 ＋
  B-46 至 B-52 共七項缺口登錄）已執行，**尚待審計官核對**，見 §5.4。
...
- **本批（補上 2a-fix 被跳過的三處更新 ＋ B-46 至 B-52 共七項缺口登錄）
  已執行完成，等待審計官核對。**
  核對通過後，批 2b 要做：**B-50（E8 不可被知情偏離繞過）與
  B-52（§6.7 搜尋對象擴為四類）優先**，其次 B-47（`--verify` 升格
  標準驗證集）、B-49（`ci_status.py` 含 badge fallback）、
  B-48（回報即 commit 的事後通道）、B-51（本地與 CI 差 1）、
  CHECK 17／18、驗證腳本 `--quiet`、B-39 落點三處同步、
  B-41 Instructions 鏡像、B-42／B-43 修正，
  以及在 `prompt-preflight.md` §3.1 登記「看板 C 節 ⇔ 交接區 §5.3」配對。
