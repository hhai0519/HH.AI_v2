...
- **Git 金鑰洩漏處置原則（依 C-02 裁決）**：若敏感 API Token 或 Secret 不慎進入 Git 歷史紀錄並推送到遠端倉庫：
  1. **第一優先（立即生效防禦）**：必須**立即將該金鑰作廢撤銷 (Revoke) 並重新輪替 (Rotate)**。已推送到遠端的金鑰一律視為公開洩漏，撤銷使其失效才是唯一絕對安全的阻斷措施。
  2. **工作區清理**：從當前工作區代碼樹中移除該敏感金鑰，並補上適當的防復發措施（如 `.gitignore`、環境變數化等），進行正常 commit 與推送。
  3. **嚴禁破壞性歷史重寫**：所有 Agent **嚴禁執行 `git push --force`、歷史重置 (History Wipe)、BFG Repo-Cleaner、git-filter-repo 或刪除/覆寫遠端分支**，嚴禁以「清理歷史」代替金鑰撤銷輪替。
  4. **歷史清理權威**：若金鑰撤銷後使用者仍希望清理遠端歷史記錄，**僅能由使用者本人決定並手動執行**。Agent 最多在使用者明確要求時提供唯讀的影響評估或操作說明，不得擅自代為執行。
