...
> [!IMPORTANT]
> **授權範圍界限**：`$$自動化$$` 為啟動深度研究（Autonomous Deep Research Loop）的領域專屬指令（Domain Command），**並非版本庫常態重構、修復或遷移的通用授權前置條件**。
> 對於版本庫程式碼與文件維護（Repository Work），使用者直接指示或宏觀審計官下達之 `GOAL_SPEC` / `EXACT_SPEC` 即已構成 Allowed Scope 內之充分執行授權。
