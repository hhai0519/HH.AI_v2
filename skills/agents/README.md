...
這些技能會用到 `authorized_mcp_tools` / `semantic_firewall` 擴充欄位，見 AGENTS.md 第 2 節。（`type` 欄位已於 2026-08-24 全面移除，`scripts/validate_skills.py` 不接受該欄位。）

## User-invoked

- **[jules-integration](./jules-integration/SKILL.md)** — 將耗時長、Token 消耗大的重構/修復任務委派給雲端 Google Jules 代理人。每日額度 100 次，需明確要求才會觸發。
...
- **[bot-account-switcher](./bot-account-switcher/SKILL.md)** — LINE 與 Telegram 官方帳號的雙平台切換工具。透過 $$Line帳號$$ 與 $$TG帳號$$ 觸發，會覆寫 .env 並重啟 PM2，需明確要求才會執行。
