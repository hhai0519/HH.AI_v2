...
function getCodeToExecute() {
  ...
  if (args.length > 0) {
    ...
    // [HIGH-01] Security check for inline code
    if (/require\s*\(|exec\s*\(|execSync\s*\(|spawn\s*\(|spawnSync\s*\(/i.test(code)) {
      console.error('❌ [HIGH-01] Security block: inline code contains forbidden modules (require, exec, spawn).');
      process.exit(1);
    }
