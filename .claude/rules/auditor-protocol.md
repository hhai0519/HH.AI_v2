...
24. **Plan-vs-Actual / 證據完整性重放（E26 — Plan-vs-Actual & Evidence Integrity Replay — B-109 M2）**：未來生產提示詞應採用 `contract_version: 2`，明列 `allowed_mutation_paths` 與 `required_mutation_paths`（`required ⊆ allowed`），設定 `max_plan_revisions: 3` 與 `execution_record_required: true`（純只讀批次 `allowed_mutation_paths: NONE`、`required_mutation_paths: NONE`、`execution_record_required: false`）。要求執行者維護 `.git/<task-id>-plan.json` 並輸出 `docs/governance/execution-record.json`；由 CI CHECK 26 重放 `base_oid..HEAD` git diff 與執行紀錄嚴格比對。提示詞必須落實 Evidence Origin（MACHINE_CAPTURED_RAW, MACHINE_DERIVED, AGENT_ASSERTED, USER_PROVIDED）與 Verification Status（VERIFIED, UNVERIFIED, NOT_ESTABLISHED, PENDING_EXTERNAL）分類，並遵循 REG-11～13 證據完整性標準。Auditor 仍負責 semantic disposition、Macro verdict 與 raw Actions 查證，執行者不得接管。
